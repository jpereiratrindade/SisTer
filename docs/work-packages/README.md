# Pacotes de Trabalho do SGE-SisTer

- [WP-SGE-CONV-01 - Convergência reproduzível do SGE](./WP-SGE-CONV-01.md)

Pacotes de trabalho conectam necessidades técnicas a arquitetura, contratos,
evidências e gates. Eles são a unidade de planejamento da fase Alfa em diante.

Antes de implementar uma mudança relevante, registre ou atualize o pacote
correspondente com:

- objetivo arquitetural;
- estágio alvo;
- ADRs relacionadas;
- contratos afetados;
- evidências esperadas;
- checks ou gates impactados;
- riscos e rollback;
- responsável técnico ou papel responsável.

## Pacotes ativos

- [WP-MVP01-00A - Baseline e vocabulário](./WP-MVP01-00A-baseline.md)
- [WP-MVP01-01 - Modelo mínimo de participação](./WP-MVP01-01-participation-model.md)
- [WP-02 - Contrato comum de subsistema](./WP-02-contrato-comum-subsistema.md)
- [WP-REF-00 - Contratos centrais da reflexividade](./WP-REF-00-reflexivity-contracts.md)
- [WP-EXEC-01 - IntegrationRun](./WP-EXEC-01-integration-run.md)
- [WP-FED-01 - Registro governado de sistemas e capacidades](./WP-FED-01-system-registry.md)
- [WP-SGE-PLAN-01 - Primeiro ciclo de planejamento reflexivo](./WP-SGE-PLAN-01.md)
- [SISTER-WEB-UX-001 v0.2.0 — Projeções de usuário, ecossistema e engenharia](./SISTER-WEB-UX-001_v0.2.0.md)
- [SISTER-WEB-QUALIFICATION-HERMETICITY-001 v0.1.0 — Qualificação hermética dos testes de superfície](./SISTER-WEB-QUALIFICATION-HERMETICITY-001_v0.1.0.md)

## Convergência de release

- [REL-BASE-01 — Baseline integrada v0.2.10](../releases/REL-BASE-01.md)
